...
/*---------------------------------------------------------------------------*/
static unsigned long strtolong(const char *data) {
...
/*---------------------------------------------------------------------------*/
void collect_common_recv(const linkaddr_t *originator,
                         uint8_t seqno,
                         uint8_t hops,
                         uint8_t *payload,
...
PROCESS_THREAD(collect_common_process, ev, data) {
    ...
    /* Send a packet every 60-62 second */
        ...
        if(ev == serial_line_event_message) {
            ...
            if(strncmp(line, "collect", 7) == 0 ||
               strncmp(line, "gw", 2) == 0) {
                ...
                if(*line == '0') {
                    NETSTACK_RDC.off(1);
                    printf("mac: turned MAC off (keep radio on): %s\n",
                            NETSTACK_RDC.name);
                }
                else {
                    NETSTACK_RDC.on();
